...
function isColabAllowed(pathname: string) {
  ...
  return COLAB_ALLOWED.some(
    (p) => platformPathname === p || platformPathname.startsWith(p + "/"),
  );
}
...
function RedirectDentistComms() {
  ...
  return (
    <Navigate
      to={{ pathname: platformPath("/dentists"), search: location.search }}
      replace
    />
  );
}
