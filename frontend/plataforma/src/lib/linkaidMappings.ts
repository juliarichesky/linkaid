...
export type StatusTicketLabel =
  (typeof STATUS_TICKET_LABELS)[StatusTicketCodigo];
...
export type ClassificacaoLabel =
  (typeof CLASSIFICACAO_LABELS)[ClassificacaoCodigo];
export type DentistaStatusCodigo = keyof typeof DENTISTA_STATUS_LABELS;
export type DentistaStatusLabel =
  (typeof DENTISTA_STATUS_LABELS)[DentistaStatusCodigo];
...
function labelPorCodigo<T extends Record<string, string>>(
  map: T,
  codigo?: string | null,
) {
  if (!codigo) return "";
...
function codigoPorLabel<T extends Record<string, string>>(
  ...
  return Object.entries(map).find(
    ([, value]) => normalize(value) === normalizedLabel,
...
export const statusTicketCodigo = (label?: string | null) =>
  codigoPorLabel(STATUS_TICKET_LABELS, label, STATUS_TICKET_ALIASES) as
    | StatusTicketCodigo
    | undefined;
...
export const tipoContatoRegistroLabel = (
  codigo?: string | null,
  nome?: string | null,
) => {
  ...
  if (
    normalizedCode === "BENEFICIARIO" ||
    (!normalizedCode && normalizedName === legacyBeneficiarioName)
  ) {
    return TIPO_CONTATO_LABELS.SOLICITANTE;
  }
  if (
    normalizedCode === "VOLUNTARIO" ||
    normalizedName === normalize("Dentista voluntário")
  ) {
...
export const editableContactTypeLabel = (label?: string | null): string =>
  EDITABLE_CONTACT_TYPE_LABELS.find((type) => type === label) ||
  TIPO_CONTATO_LABELS.SOLICITANTE;
...
export const classificacaoCodigo = (label?: string | null) =>
  codigoPorLabel(CLASSIFICACAO_LABELS, label) as
    | ClassificacaoCodigo
    | undefined;
...
export const dentistaStatusCodigo = (label?: string | null) =>
  codigoPorLabel(DENTISTA_STATUS_LABELS, label) as
