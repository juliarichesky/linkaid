...
const allTransactions: Transaction[] = [
  {
    ...
    history: [
      { date: "05/04/2025 09:00", action: "Transação criada", user: "Sistema" },
      {
  ...
  },
  {
    ...
    responsible: "Ana Costa",
    history: [
      { date: "03/04/2025 08:00", action: "Transação criada", user: "Sistema" },
    ],
  },
  {
    id: "FIN-004",
    date: "02/04/2025",
    entity: "Aluguel sala",
    description: "Aluguel mensal clínica",
    value: "R$ 3.200,00",
    type: "Despesa",
    status: "Pago",
    ...
    history: [
      {
        date: "02/04/2025 07:00",
        action: "Débito automático",
        user: "Sistema",
      },
    ],
  },
  {
    id: "FIN-005",
    date: "01/04/2025",
    entity: "Maria Oliveira",
    description: "Doação eventual",
    value: "R$ 500,00",
    type: "Receita",
    status: "Confirmado",
    category: "Doação",
    ...
    responsible: "Sistema",
    history: [
      { date: "01/04/2025 16:00", action: "Transação criada", user: "Sistema" },
      {
        date: "01/04/2025 16:05",
  ...
  },
  {
    ...
    date: "28/03/2025",
    entity: "Empresa XYZ",
    description: "Doação corporativa",
    value: "R$ 15.000,00",
    type: "Receita",
    status: "Confirmado",
    category: "Doação",
    ...
    responsible: "Ana Costa",
    history: [
      { date: "28/03/2025 09:00", action: "Transação criada", user: "Sistema" },
    ],
  },
  {
    id: "FIN-008",
    date: "25/03/2025",
    entity: "Seguradora Dental",
    description: "Seguro equipamentos",
    value: "R$ 800,00",
    type: "Despesa",
    status: "Pago",
    category: "Seguro",
  ...
  },
  {
    id: "FIN-010",
    date: "20/03/2025",
    entity: "Fornecedor Insumos",
    description: "Luvas e materiais",
    value: "R$ 2.300,00",
    type: "Despesa",
    status: "Pago",
    category: "Material",
...
export default function Financial() {
  ...
  const filtered = transactions.filter((t) => {
    ...
    return (
      matchSearch && matchStatus && matchCategory && matchType && matchDentist
    );
  });
  ...
  const handleCreateTx = () => {
    if (!newValue || !newEntity || !newCategory) {
      toast.error(
        "Preencha os campos obrigatórios: Entidade, Valor e Categoria",
      );
      return;
    }
    const newTx: Transaction = {
      ...
      history: [
        {
          date: `${newDate || new Date().toLocaleDateString("pt-BR")} ${new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}`,
          action: "Transação criada",
          user: newResponsible || "Sistema",
        },
      ],
    };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Gestão de recursos e declaração
          </p>
        </div>
            ...
            {paginated.map((t) => (
              ...
              >
                ...
                <TableCell>
                  <Badge
                    variant="outline"
                    className="text-xs min-w-[80px] flex items-center justify-center"
                  >
                    {t.category}
                  </Badge>
                </TableCell>
                <TableCell
                  className={`font-medium text-sm ${t.type === "Receita" ? "text-success" : "text-destructive"}`}
                >
                  {t.type === "Despesa" ? `- ${t.value}` : t.value}
                </TableCell>
                <TableCell>
                  <Badge
                    variant={t.type === "Receita" ? "default" : "secondary"}
                    className="min-w-[72px] flex items-center justify-center"
                  >
                    {t.type}
                  </Badge>
                </TableCell>
                <TableCell>
                  <Badge
                    variant="outline"
                    className={`text-xs min-w-[80px] flex items-center justify-center ${t.status === "Pendente" ? "border-warning text-warning" : ""}`}
                  >
                    {t.status}
                  </Badge>
                </TableCell>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-mono font-medium">{selectedTx.id}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedTx.entity}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Categoria</p>
                  <Badge variant="outline">{selectedTx.category}</Badge>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Valor</p>
                  <p
                    className={`font-bold text-lg ${selectedTx.type === "Receita" ? "text-success" : "text-destructive"}`}
                  >
                    {selectedTx.value}
                  </p>
                </div>
                <div>
                  ...
                  <Badge
                    variant={
                      selectedTx.type === "Receita" ? "default" : "secondary"
                    }
                  >
                    {selectedTx.type}
                  </Badge>
                ...
                <div>
                  ...
                  <Badge variant="outline">{selectedTx.status}</Badge>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedTx.receipt}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Responsável</p>
                  <p className="font-medium">{selectedTx.responsible}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">
                    Dentista Vinculado
                  </p>
                  <p className="font-medium">{selectedTx.dentist}</p>
                </div>
              </div>
              <Separator />
              <div>
                <p className="text-sm font-medium flex items-center gap-1.5 mb-3">
                  <Clock className="w-4 h-4 text-muted-foreground" /> Histórico
                  de Alterações
                </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="R$ 0,00"
                  value={newValue}
                  onChange={(e) => setNewValue(maskCurrency(e.target.value))}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
